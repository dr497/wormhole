...
import TransferInitiator, {
  defaultCoinInfo,
  TransferInitiatorData,
} from "../components/TransferInitiator";
...
if (window.ethereum === undefined) {
  alert(
    "Please install the MetaMask extension before using this experimental demo web UI"
  );
}
...
async function approveAssets(asset: string, amount: BigNumberish) {
  let e = Erc20Factory.connect(asset, signer);
  try {
    message.loading({
      content: "Signing transaction...",
      key: "eth_tx",
      duration: 1000,
    });
...
function Assistant() {
  ...
  let nextStep = (from: string) => {
    ...
    if (from == "approve") {
      lockAssets(
        transferData.fromCoinInfo.address,
        transferData.amount,
        transferData.toAddress,
        transferData.toNetwork
      );
    } else if (from == "lock") {
      // Await approvals or allow to submit guardian shit
      if (
        fromNetwork == ChainID.ETH &&
        transferData.toNetwork == ChainID.SOLANA
      ) {
        awaitCompletionEth();
      } else if (
        fromNetwork == ChainID.SOLANA &&
        transferData.toNetwork == ChainID.ETH
      ) {
  ...
  const lockAssets = async function(
    asset: string,
    amount: BigNumberish,
    recipient: Arrayish,
  ...
  const approveAssets = async function(asset: string, amount: BigNumberish) {
    let e = Erc20Factory.connect(asset, signer);
    try {
      setLoading({
        ...loading,
  ...
  const solanaTransfer = async () => {
    ...
    let { ix: lock_ix, transferKey } = await bridge.createLockAssetInstruction(
      wallet.publicKey,
      new PublicKey(transferData.fromCoinInfo.address),
      new PublicKey(transferData.fromCoinInfo.mint),
      new BN(transferData.amount.toString()),
      transferData.toNetwork,
      transferData.toAddress,
      {
    ...
    let ix = spl.Token.createApproveInstruction(
      TOKEN_PROGRAM,
      new PublicKey(transferData.fromCoinInfo.address),
      await bridge.getConfigKey(),
      wallet.publicKey,
      [],
  ...
  const awaitCompletionEth = () => {
    ...
    let accountChangeListener = c.onAccountChange(
      new PublicKey(transferData.toAddress),
      () => {
        if (completed) return;
  ...
  const steps = [
    {
      ...
      content: (
        <>
          <TransferInitiator
            onFromNetworkChanged={setFromNetwork}
            dataChanged={(d) => {
              setTransferData(d);
            }}
          />
  ...
  if (fromNetwork == ChainID.ETH) {
    if (
      transferData.fromCoinInfo &&
      transferData.fromCoinInfo.allowance.lt(transferData.amount)
    ) {
      steps.push({
        title: "Approval",
        content: <></>,
